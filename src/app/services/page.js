...
export default function ServicesPage() {
      ...
      {Object.keys(services).map((category) => (
        <button
          ...
          className={`py-2 px-6 rounded-lg font-semibold transition-colors duration-200 ${
            activeTab === category ? 'bg-accent text-gray-900' : 'bg-white text-gray-700'
          }`}
        >
  ...
  const renderServiceCards = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {services[activeTab].map((service, index) => (
        <div key={index} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-2xl transition-shadow duration-200">
  ...
  return (
    <section className="py-16 bg-brown text-gray-900"> {/* Aplicando el fondo marrón aquí */}
      <div className="container mx-auto text-center">
        <h1 className="text-5xl font-bold mb-8 text-primary">Nuestros Servicios</h1>
        <p className="text-lg mb-12 max-w-2xl mx-auto text-gray-700">
          Explora todos los servicios de alta calidad que ofrecemos en Elevation Lounge. Desde cortes de cabello hasta tratamientos capilares, tenemos algo para cada caballero.
        </p>
